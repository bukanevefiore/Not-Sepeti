...
// open the database
  ...
  Future<List<Kategori>> kategoriListesiniGetir() async {
    ...
    var kategorilerListesi = await List<Kategori>();
    for(Map map in kategorileriIcerenMapListesi){
      kategorilerListesi.add(Kategori.fromMap(map));
    }
  ...
  Future<int> kategoriSil(int kategoriID) async {
    var db=await _getDataBase();
    var sonuc = await db.delete("kategori", where: 'katogoriID = ?', whereArgs: [kategoriID]);
    return sonuc;
  }
  // notlar mapi
  Future<List<Map<String, dynamic>>> notlariGetir() async {
    var db =await _getDataBase();
    var sonuc = await db.rawQuery('select * from "not" inner join kategori on kategori.katogoriID = "not".kategoriID order by notID Desc;');
    return sonuc;
  }
